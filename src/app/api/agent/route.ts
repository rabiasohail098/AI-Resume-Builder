...
export async function POST(req: NextRequest) {
    try {
        const { field, context } = await req.json() as AgentRequest;
        
        if (!field) {
            return NextResponse.json(
                { error: "Field is required" } as AgentResponse,
                { status: 400 }
            );
        }
        ...
        if (!response.choices || response.choices.length === 0) {
            return NextResponse.json(
                { error: "No response from Model." } as AgentResponse,
                { status: 500 }
            );
        }
        
        const suggestion = response.choices[0].message.content;
        return NextResponse.json({ suggestion } as AgentResponse);
        ...
        return NextResponse.json(
            { error: errorMessage } as AgentResponse,
            { status: 500 }
        );
    }
}